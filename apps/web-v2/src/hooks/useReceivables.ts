...
export function useReceivables() {
  ...
  const fetch = useCallback(async () => {
    ...
    catch (e) { if (e instanceof ApiError && e.status === 401) return; console.error(e); toast.error('Failed to load receivables') }
    setIsLoading(false)
  }, [companyId])
  ...
  const totalCollected = paid.reduce((s, r) => s + r.amount_cents, 0)
  ...
  const collect = async (id: string) => { await update(id, { status: 'paid' }) }

  return { isLoading, items, active, paid, totalOwed, totalCollected, overdueCount, create, update, remove, collect, refetch: fetch }
}
