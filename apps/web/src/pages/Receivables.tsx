  ...
  return (
    <div className="min-h-screen pb-4">
      <div className="flex items-center p-4">
        ...
        <h1 className="text-xl font-bold flex-1">គេជំពាក់ខ្ញុំ</h1>
      ...
      {isLoading ? (
        ...
        <p className="text-center text-gray-400 py-12">គ្មានអ្នកជំពាក់</p>
      ) : (
        <div className="px-4 space-y-2">
          {items.map((item: { id: string; contact_name: string; amount_cents: number; status: string }) => (
            <div key={item.id} className={`flex items-center p-3 rounded-xl border ${item.status === 'paid' ? 'border-green-100 bg-green-50' : 'border-orange-100 bg-white'}`}>
              <div className="flex-1">
                <p className="font-medium">{item.contact_name}</p>
                <p className={`text-lg font-bold ${item.status === 'paid' ? 'text-green-600 line-through' : 'text-orange-600'}`}>
