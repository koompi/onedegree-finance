...
export default function CurrencyInput({ onChange }: Props) {
  ...
  const press = (key: string) => {
    ...
    if (key === 'DEL') {
      ...
      if (currency === 'KHR') return // no decimals for KHR
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        <span className={`flex-1 text-4xl font-bold text-right pr-2 ${isEmpty ? 'text-gray-300' : 'text-gray-900'}`}>
          {display}
        </span>
        <button
          type="button"
        ...
        {keys.flat().map((key, i) => {
          ...
          return (
            <button
              ...
              type="button"
              onClick={() => press(key)}
              className={`py-4 rounded-2xl text-xl font-semibold transition-all duration-100 active:scale-95 ${
                key === 'DEL'
